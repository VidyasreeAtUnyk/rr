...
// Helper to emit status for ideaId by looking up title and user
async function emitStatusForIdea(ideaId, payload) {
	try {
		const [rows] = await pool.execute(
			`SELECT t.user_id as userId, p.title_id as titleId, p.id as paintingId
			 FROM paintings p JOIN ideas i ON p.idea_id = i.id JOIN titles t ON p.title_id = t.id
			 WHERE p.idea_id = ? LIMIT 1`,
			[ideaId]
		);
		if (rows && rows.length > 0) {
			const { userId, titleId, paintingId } = rows[0];
			emitTo(userId, String(titleId), 'painting', { paintingId, ideaId, ...payload });
		}
	} catch (e) {
		// best-effort
	}
...
async function generateImage(ideaId, prompt, references = []) {
	...
	try {
		...
		await emitStatusForIdea(ideaId, { status: 'completed', image_url: `uploads/${fileName}` });
		...
		try {
			...
			if (failureUpdateParams.some(p => p === undefined)) {
				...
				await emitStatusForIdea(ideaId, { status: 'failed', error_message: errorMsg.substring(0, 255) });
