...
const { emitTo } = require('../services/eventBus');
...
async function generatePaintings(req, res) {
	...
	try {
		...
		const [titleRows] = await pool.execute(
			'SELECT id, user_id, title, instructions FROM titles WHERE id = ?',
			titleParams
		);
    ...
    console.log("titleRows", titleRows);
		...
		for (let i = 0; i < quantity; i++) {
			...
			// Emit pending placeholder event
...
async function getPaintings(req, res) {
    ...
    console.log("paintingQuery", paintingQuery);
    ...
    console.log("paintingRows", paintingRows);
